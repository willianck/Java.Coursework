...
public class ScotlandYardModel implements ScotlandYardGame, Consumer<Move>, MoveVisitor, Spectator {
	...
	public ScotlandYardPlayer playerNow(Colour colour) {
	...
	//Returns true if the gam is over
	@Override
	...
	private boolean mrxCornered() {
		return players.get(currentPlayer).isMrX() && validMove(players.get(currentPlayer)).isEmpty();
	}
